...
def parse_args():
    ...
    parser.add_argument('--right', required=True, default=path.join(getcwd(),'right.mp4'),
                        help='Video for the right half of the rink' )
    parser.add_argument('--left', required=True, default=path.join(getcwd(),'left.mp4'),
                        help='Video for the left half of the rink' )
...
class CutListPlayer:
    def __init__( self, cutlist, right = None, left = None ):
...
def main():
    ...
    cutlist = [ [opts.left, 0, True ] ] 

    with CutListPlayer(cutlist, opts.right, opts.left) as player:
        player.draw()
        running = 0
